#!/bin/bash
# Start InventoryManagementService with two New Relic agents (two separate JVM processes)
# Each process reports to a different NR account.
#
# Ports: 9003 (Account 1), 9004 (Account 2)
#
# Required env vars:
...
APP_JAR="build/libs/InventoryManagementService-1.0-SNAPSHOT.jar"
...
echo "Starting InventoryManagementService with NR Agent -> Account 1 (port 9003)..."
...
echo "Starting InventoryManagementService with NR Agent -> Account 2 (port 9004)..."
java -javaagent:$NR_AGENT \
...
echo "Both instances running: PID1=$PID1 (Account1:9003), PID2=$PID2 (Account2:9004)"
